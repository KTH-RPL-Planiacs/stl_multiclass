...
dict_classes_trajectory = pickle.load(open("user_study_data/dict_classes_trajectory.pkl", "rb" ))
...
for train_index, test_index in sss.split(X_id, y):
    ...
    X_train = np.array([np.array(dict_trajectories[t])[:125] for t in X_id_train])
    X_test  = np.array([np.array(dict_trajectories[t])[:125] for t in X_id_test])
    ...
    nn_model.save('user_study_data/output_models/cv/nn/nn_model_'+str(fold)+'.pkl')
    with open('user_study_data/output_models/cv/nn/nn_model_time_'+str(fold)+'.pkl', "wb") as pfile:
        pickle.dump(end_time-start_time, pfile)
    
       
    nn_model = keras.models.load_model('user_study_data/output_models/cv/nn/nn_model_'+str(fold)+'.pkl')
    total_time = pickle.load(open('user_study_data/output_models/cv/nn/nn_model_time_'+str(fold)+'.pkl', "rb" ))
    ...
    start_time = time.time()
    #For the different experiments, change the 2 last parameters (here, max_depth=5 and stl_diff=True)
    dtlearn = DTLearn(dict_trajectories, dict_trajectories_classes, list_classes, -10, -50, 20, 10, 50, 100, 5, True)
    tree = dtlearn.recursiveGenerateTree(X_id_train)
    end_time   = time.time()
    

    with open('user_study_data/output_models/cv/stl_dt/stl_dt_'+str(fold)+'.pkl', "wb") as pfile:
        pickle.dump(end_time-start_time, pfile)
    with open("user_study_data/output_models/cv/stl_dt/stl_dt_"+str(fold)+".dill", "wb") as pfile:
        dill.dump(tree, pfile)
    
    # tree = dill.load(open("user_study_data/output_models/cv/stl_dt/stl_dt_"+str(fold)+".dill", "rb" ))
    # total_time = pickle.load(open('output_models/cv/nn/nn_model_time_'+str(fold)+'.pkl', "rb" ))
