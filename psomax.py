...
def pso(func, lb, ub, ieqcons=[], f_ieqcons=None, args=(), kwargs={}, 
        swarmsize=150, omega=0.5, phip=0.5, phig=0.5, maxiter=100, 
        minstep=1e-8, minfunc=1e-8, debug=False, max_no_improvement=10):
    """
    ...
    max_no_improvement : int
        Maximum number of iterations with no improvements. Above, the algorithm terminates
        (Default: 5)
...
def pso_maximize(func, lb, ub, ieqcons=[], f_ieqcons=None, args=(), kwargs={}, 
    ...
    while it<=maxiter:
        ...
        for i in range(S):
            ...
            # Compare particle's best position (if constraints are satisfied)
            if fp[i]>fx and is_feasible(x[i, :]):
                p[i, :] = x[i, :].copy()
                fp[i] = fx
